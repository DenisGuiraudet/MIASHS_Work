...
function writeListOfBlocks() {
  ...
  xmlhttp.onreadystatechange = function() {
      if (this.readyState == 4 && this.status == 200) {
        ...
        for (let section of jason) writeBlock(section);

        createQuestion();

        console.log(getTextFamily(document.querySelector("#q1").parentNode));
        console.log(getTextFamily(document.querySelector("div.listOfBlocks")));
...
function writeBlock(section) {
  ...
  //
  for (var i = 0; i < section.Reponses.idR.length; i++) {
    let checkbox = document.createElement("input");
    checkbox.type = "checkbox";
    checkbox.id = section.Reponses.idR[i];
    checkbox.addEventListener("click", checkChecked, false);
    rep.appendChild(checkbox);
    rep.appendChild(document.createTextNode(" " + section.Reponses.intituleR[i]));
